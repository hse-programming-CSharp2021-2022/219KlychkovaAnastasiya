...
namespace MultithreadingAndSynchronizationTask
{
    internal class BadSolution
    {
        static void Main(string[] args)
        {
            ...
            {
                Bee createdBee = new Bee(pot, bear,i.ToString());
                bees.Add(createdBee);
    ...
    class Pot
    {
        ...
        public void IncrementFullness(string beeName)
        {
            mutex.WaitOne();
            fullness++;
            Console.WriteLine($"Bee {beeName} filled up to {fullness}");
            mutex.ReleaseMutex();
        }
    ...
    class Bee
    {
        ...
        public static object wait = new();
        private static Mutex mutex = new Mutex();
        public Bee(Pot pot, Bear bear,string name)
        {
            this.pot = pot;
            this.bear = bear;
            this.name = name;
